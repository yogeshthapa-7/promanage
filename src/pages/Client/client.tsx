...
export default function ClientPage() {
  ...
  const {
    ...
    fetcher: (params) => fetchClientsPage({ 
      ...params, 
      search: searchQuery  
    }),
    initialPageSize: 20,
  ...
  const handleDelete = async (client: Client) => {
    Modal.confirm({
      title: 'Delete Client',
      content: `Are you sure you want to delete "${client.clientName || 'this client'}"?`,
      okText: 'Delete',
      okType: 'danger',
      onOk: async () => {
        try {
      ...
      <div className="mt-6">
        ...
        {loading ? (
          viewMode === 'grid' ? (
            ...
            <TableSkeleton columns={6} rows={6} message="Loading clients..." />
          )
        ) : clients.length === 0 ? (
