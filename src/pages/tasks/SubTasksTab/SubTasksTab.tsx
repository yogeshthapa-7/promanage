  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground">({selectedTask.TaskCode})</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-200 bg-white p-12 text-center">
          <p className="text-sm text-slate-500 mb-3">This task does not have any subtasks yet.</p>
          <button
            onClick={() => {}}
              ...
              <div key={sub.SubTaskInfoID} className="rounded-xl border border-slate-200 bg-white p-4 flex flex-col gap-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {sub.SubTaskCode && <p className="text-sm text-muted-foreground font-mono mt-0.5">{sub.SubTaskCode}</p>}
                  </div>
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${priorityClass}`}>{sub.PriorityName}</span>
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${statusClass}`}>{sub.WorkStatusName}</span>
                  </div>
                </div>

                <p className="text-sm text-slate-500 line-clamp-3">{selectedTask.Description || "No description available."}</p>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm text-muted-foreground">Weightage: {sub.Weightage}</span>
                </div>
              </div>
            );
          })}
