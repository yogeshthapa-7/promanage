...
import { LayoutDashboard, FolderKanban, CheckSquare, Users, ChevronLeft, ChevronRight, LogOut, ChartLine, Building, Building2, User, File, Wallet2, CreditCard, Handshake, UserCircle, Tag, MapPin, Calendar, X, } from 'lucide-react';
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-shrink-0 px-3 pb-3 relative">
          <button
            onClick={() => setUserMenuOpen(!userMenuOpen)}
            className="group relative w-full flex items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.025] text-slate-300 hover:text-white hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-200 px-3.5 py-3"
          >
      ...
      <aside className="hidden md:flex flex-col h-screen flex-shrink-0 relative z-20 overflow-hidden border-r transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]" style={{ width: collapsed ? '80px' : '300px', background: 'linear-gradient(180deg, #07152f 0%, #0a1b3d 45%, #08152e 100%)', borderColor: 'rgba(148, 163, 184, 0.12)', boxShadow: '8px 0 35px rgba(2, 8, 23, 0.25), inset -1px 0 rgba(255,255,255,0.025)' }}>
        ...
        <div className="flex-shrink-0 px-3 pb-3 relative">
          <button
            onClick={() => setUserMenuOpen(!userMenuOpen)}
            className={`group relative w-full flex items-center rounded-xl border border-white/[0.07] bg-white/[0.025] text-slate-300 hover:text-white hover:bg-white/[0.06] hover:border-white/[0.12] transition-all duration-200 ${collapsed ? 'justify-center py-3' : 'gap-3 px-3.5 py-3'}`}
          >
            <div className="relative flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-violet-600 text-white text-sm font-bold shadow-lg shadow-blue-900/30">
              {user?.name ? user.name.charAt(0).toUpperCase() : <User size={16} />}
            </div>
